...
<head>
    ...
    <script>
    function silmeOnayi(silmeLinki) {
        Swal.fire({
            title: 'Silmek istediğine emin misin?',
            text: "Bu gönderi kalıcı olarak silinecek!",
            icon: 'warning',
            showCancelButton: true,
            
            // TEMA AYARLARI BURADA
            // Butonların varsayılan çirkin stillerini kapatıyoruz
            buttonsStyling: false, 
            
            // Senin temana uygun CSS sınıfları (Tailwind)
            customClass: {
                popup: 'rounded-2xl p-6', // Pencere kenarları yuvarlatıldı
                title: 'text-gray-800 text-xl font-bold',
                htmlContainer: 'text-gray-500',
                confirmButton: 'bg-cyan-500 hover:bg-blue-600 text-white font-bold py-2.5 px-6 rounded-xl shadow-md transition duration-200 mx-2', // Kırmızı, yuvarlak buton
                cancelButton: 'bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold py-2.5 px-6 rounded-xl transition duration-200 mx-2' // Gri, soft iptal butonu
            },
            
            confirmButtonText: 'Evet, Sil',
            cancelButtonText: 'Vazgeç',
            focusCancel: true // Yanlışlıkla silmemesi için odak "Vazgeç"te olsun
            
        }).then((result) => {
            if (result.isConfirmed) {
                // Silme işlemi sırasında kullanıcıya geri bildirim verelim
                Swal.fire({
                    title: 'Siliniyor...',
                    timer: 1500,
                ...
                // Yönlendirme
                window.location.href = silmeLinki;
            }
        })
    }
</script>
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <?php if ($is_mine): ?>
                                            ...
                                            onclick="silmeOnayi('<?= site_url('profile/delete_post/' . $post->id); ?>')"
                                            class="p-1 text-slate-400 hover:text-cyan-500 hover:bg-red-50 rounded-lg transition-all"
                                            title="Gönderiyi sil">
